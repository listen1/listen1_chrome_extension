...
/* global qq, xiami, netease */
/* global getParameterByName, myplaylist */
		...
		return {
			get: function (url) {
				...
				if (url.search('/playlist') != -1) {
					...
					provider = getProviderByItemId(list_id);
					return provider.get_playlist(url, $http, $httpParamSerializerJQLike);
				}
				if (url.search('/search') != -1) {
					source = getParameterByName('source', url);
					provider = getProviderByName(source);
				...
				if (url.search('/lyric') != -1) {
					...
					provider = getProviderByItemId(track_id);
			...
			post: function (request) {
				...
				if (request.url.search('/clone_playlist') != -1) {
					list_id = getParameterByName('list_id', url + '?' + request.data);
					provider = getProviderByItemId(list_id);
					url = '/playlist?list_id=' + list_id;
					return {
						success: function (fn) {
				...
				if (request.url.search('/add_myplaylist') != -1) {
					list_id = getParameterByName('list_id', url + '?' + request.data);
					track_json = getParameterByName('track', url + '?' + request.data);
				...
				if (request.url.search('/remove_track_from_myplaylist') != -1) {
					list_id = getParameterByName('list_id', url + '?' + request.data);
					track_id = getParameterByName('track_id', url + '?' + request.data);
				...
				if (request.url.search('/parse_url') != -1) {
					url = getParameterByName('url', url + '?' + request.data);
