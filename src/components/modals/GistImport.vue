  ...
  <DefaultModal>
    ...
    <template #body>
      <ul class="text-left">
        ...
        <li class="h-24 cursor-pointer rounded p-2 hover:bg-dialog-hover" @click="uploadInput.click()">
          <img class="float-left mr-4 h-20 w-20 rounded" src="/images/mycover.jpg" />
          <h2>{{ t('_RECOVER_FROM_LOCAL_FILE') }}</h2>
        </li>
        <p class="mb-3 font-semibold">{{ t('_RECOVER_FROM_GITHUB_GIST') }}</p>
        <li v-for="(backup, index) in myBackup" :key="index" class="h-24 cursor-pointer rounded p-2 hover:bg-dialog-hover" @click="recover(backup.id)">
          <img class="float-left mr-4 h-20 w-20 rounded" src="/images/mycover.jpg" />
