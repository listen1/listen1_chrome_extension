...
export class Player {
  ...
  insertAudioByDirection(audio, to_audio, direction) {
    ...
    this.playlist = Player.array_move(this.playlist, index, insertIndex + offset);
    const foundOriginTrackIndex = this.playlist.findIndex((i) => i.id === originTrack.id);
  ...
  adjustVolume(inc) {
    this.volume = inc ? Math.min(this.volume + 0.1, 1) : Math.max(this.volume - 0.1, 0);
    this.sendVolumeEvent();
    this.sendFrameUpdate();
  }
...
export const threadPlayer = new Player();
threadPlayer.setRefreshRate();
threadPlayer.finishInit();
...
if ('mediaSession' in navigator) {
  ...
  mediaSession.setActionHandler('seekforward', (details) => {
    ...
    const newTime = Math.min(currentHowl.seek() + skipTime, currentHowl.duration());
